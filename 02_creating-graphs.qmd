...
While `ggplot2` attempts to find sensible scales for the plot, sometimes you might want to restrict this or set a different number of breaks. For continuous scales, we can set this using `scale_x_continuous()` and `scale_y_continuous()`. For discrete scales, it's unsurprisingly, `scale_x_`discrete`()` and `scale_y_`discrete`()`.
